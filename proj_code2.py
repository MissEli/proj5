...
energies = [577+Eloss[0],1815+Eloss[3]]
Etrue = [0.577,1.4, 1.464,1.815, 2.05,2.73, 4.75]
Etrue = [element*1000 for element in E]
...
def E_calib(k,m,xdata,ydata,peaks,stds):
    energies = [x*k+m for x in xdata]
    E_peaks = [peak*k+m for peak in peaks]
    E_std = [std*k+m for std in stds]
    plt.figure(figsize=(12,10))
    plt.plot(energies,ydata,'-')
    plt.xlabel('Energy [keV]')
    plt.ylabel('Events')
    output = np.vstack((E_peaks,E_std))
    return output
